...
app = FastAPI(title="Учёт заявок на ремонт бытовой техники")
...
def get_user_role_display(request: Request) -> str:
    """Получает читаемое название роли из cookie, преобразуя код роли."""
    ...
    # Получаем код роли из cookie и преобразуем в читаемое название
    user_role = request.cookies.get("user_role")
    if user_role:
        return code_to_role_display(user_role)
    return "Гость"
...
def index(request: Request, db: Session = Depends(get_db)):
    ...
    for req in requests:
        all_comments = crud_requests.get_comments_by_request(db, req.requestID)
        req.comments = all_comments
        # Разделяем на обычные комментарии и обращения за помощью
        # Безопасная проверка наличия поля is_help_request
        req.regular_comments = [c for c in all_comments if getattr(c, 'is_help_request', '0') != "1"]
        req.help_comments = [c for c in all_comments if getattr(c, 'is_help_request', '0') == "1"]
    
    users = {u.userID: u.fio for u in db.query(User).all()}

    user_role_display = get_user_role_display(request)
...
def assign_form(
    ...
    specialists = db.query(User).filter(User.type == "Мастер").all()
...
def assign_master(
    ...
    if not updated:
        raise HTTPException(status_code=404, detail="Не удалось назначить мастера")
...
def edit_request(
    ...
    # Обновляем данные
    req.applianceType = applianceType
    req.applianceModel = applianceModel
...
def comment_form(
    ...
    # Убедимся, что мастер назначен на эту заявку
    user_id = int(request.cookies.get("user_id"))
    req = crud_requests.get_request(db, request_id)
...
# === Продление срока заявки (для менеджера) ===
@app.get("/extend/{request_id}", response_class=HTMLResponse)
def extend_deadline_form(
    request_id: int,
    request: Request,
    db: Session = Depends(get_db),
    _: bool = Depends(require_role("manager", "admin"))
    ...
    return templates.TemplateResponse("extend_deadline.html", {
        "request": request,
        "req": req,
        "client_fio": client_fio,
        "user_role": request.cookies.get("user_role"),
...
def extend_deadline(
    ...
    if client_agreement is None or client_agreement not in ["yes", "on"]:
        req = crud_requests.get_request(db, request_id)
        client = db.query(User).filter(User.userID == req.clientID).first()
        client_fio = client.fio if client else f"ID{req.clientID}"
        return templates.TemplateResponse("extend_deadline.html", {
            ...
            "req": req,
            "client_fio": client_fio,
            "error": "Необходимо согласие клиента для продления срока",
            "user_role": request.cookies.get("user_role"),
            "user_role_display": get_user_role_display(request)
        }, status_code=400)
    
    updated = crud_requests.extend_request_deadline(db, request_id, new_date)
    if not updated:
        raise HTTPException(status_code=404, detail="Не удалось продлить срок")
    return RedirectResponse(url="/", status_code=303)

# === Обращение мастера за помощью ===
@app.get("/help/{request_id}", response_class=HTMLResponse)
def help_request_form(
    request_id: int,
    request: Request,
    db: Session = Depends(get_db),
    user_role: str = Depends(require_role("specialist"))
    ...
    return templates.TemplateResponse("help_request.html", {
        "request": request,
        "request_id": request_id,
        "user_role": user_role,
        "user_role_display": get_user_role_display(request)
...
def create_help_request(
    ...
    crud_requests.create_help_request(db, request_id, user_id, message)
    return RedirectResponse(url="/", status_code=303)

# === Просмотр обращений за помощью (для менеджера) ===
@app.get("/help-requests", response_class=HTMLResponse)
def help_requests_list(
    request: Request,
    db: Session = Depends(get_db),
    _: bool = Depends(require_role("manager", "admin"))
):
    help_requests = crud_requests.get_help_requests(db)
    users = {u.userID: u.fio for u in db.query(User).all()}
    requests_dict = {r.requestID: r for r in db.query(RepairRequest).all()}
    
    return templates.TemplateResponse("help_requests.html", {
        "request": request,
