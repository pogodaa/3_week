...
class User(Base):
    ...
    type = Column(String, nullable=False)  # "Заказчик", "Оператор", "Мастер", "Менеджер"
...
class RepairRequest(Base):  #  было Request
    ...
    applianceType = Column(String, nullable=False)
    applianceModel = Column(String, nullable=False)
    ...
    expectedCompletionDate = Column(String, nullable=True)  # Ожидаемая дата завершения (для продления срока)
...
class Comment(Base):
    ...
    created_at = Column(String, default=datetime.now().strftime("%Y-%m-%d %H:%M"))
    is_help_request = Column(String, default="0")  # "1" если это обращение за помощью, "0" - обычный комментарий