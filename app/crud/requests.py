...
def extend_request_deadline(db: Session, request_id: int, new_date: str):
    """Продлевает срок выполнения заявки."""
    request = get_request(db, request_id)
    if request:
        request.expectedCompletionDate = new_date
        db.commit()
        db.refresh(request)
    return request

def create_help_request(db: Session, request_id: int, master_id: int, message: str):
    """Создает обращение мастера за помощью."""
    db_comment = Comment(
        message=message,
        masterID=master_id,
        requestID=request_id,
...
def get_help_requests(db: Session):
    """Получает все обращения за помощью для менеджера."""
    return db.query(Comment).filter(Comment.is_help_request == "1").all()

def get_help_requests_by_request(db: Session, request_id: int):
    """Получает обращения за помощью по конкретной заявке."""
    return db.query(Comment).filter(
        Comment.requestID == request_id,
        Comment.is_help_request == "1"
    ).all()